...
const RELEASE_ORDER = ["weekly", "daily", "schedule", "rolling"];
...
export function evaluate(rule, usage, nowMs, { counting = false } = {}) {
  ...
  if (schedule.blocked) releases.schedule = schedule.untilMs;
  ...
  if (exhausted) {
    ...
    if (schedule.nextStartMs !== null) candidates.push(schedule.nextStartMs);
    if (pass.active) candidates.push(pass.endsAtMs);
    if (candidates.length > 0) nextChangeAtMs = Math.min(...candidates);
  }

  return { exhausted, reason, binding, remainingMs, unlockAtMs, nextChangeAtMs, caps, schedule, pass };
}
