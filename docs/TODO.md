...
- [x] **Deleting every rule resurrects the defaults.** `settings.js` treats `rules: []` as
...
- [x] **`load()` bypasses the write chain and loses in-flight usage.** On a settings save,
...
- [x] **Delete a rule and re-add one with the same name in one save → it inherits the old
...
- [x] **Tab-level mute probably does not stop the clock, contrary to DESIGN.md §3.** Per MDN,
  `tab.audible` stays true while the tab is muted via the speaker icon; `isCounting` never
  consults `mutedInfo` even though it sits in the event filter. **Hand-checked on Firefox
  (2026-09-03): not a bug.** Muting a playing tab via its speaker icon does stop the counter —
  the observer sees it without consulting `mutedInfo`, contrary to the MDN reading. Behaviour
  matches DESIGN.md §3 as written; no change made.

- [x] **`onExceed: "close"` on the only tab closes the window — and the last window closes
  Firefox** (default `closeWindowWithLastTab`). The shipped Instagram default uses `close`, so
  this is reachable. Fixed, but not by navigating to `about:newtab` as suggested — `tabs.update`
  rejects privileged about: URLs as illegal. Instead the enforcer creates a fresh tab in the
  window before removing the last one — `extension/background/enforcer.js:38`.

## Duplication the code's own structure argues against

- [x] **`blocked.js` reimplements `clock()`** — with ceil semantics, i.e. exactly
...
- [x] **`popup.js` redefines `MODE_HINT`** while `MODES[].hint` in `rules.js` carries the same
...
- [x] **Subdomain shadowing is not caught by validation.** `validateRule` compares domains by
...
- [x] **The block page polls the background every 5s forever**, pinning the event page alive —
...
- [x] **Trailing-dot hostname bypass.** `https://youtube.com./` yields hostname
  `"youtube.com."`, which matches nothing. Strip a trailing `.` in `hostnameOf` —
  `extension/common/rules.js:66`.

- [x] **Async `onMessage` listener claims the response channel for every message.** Harmless
...
- [x] **`setLimits()` skips validation.** `budgetSec: 0` yields `exhausted: true` with
...
- [x] **Cover `store.js`'s lifecycle logic with a `globalThis.browser` stub** (~40 lines, no
...
- [x] DESIGN.md §6 (storage sketch) shows `usage:<id> → { b: {...}, blockedUntil }`; there is no
  `blockedUntil` — the unlock instant is derived by `unlockAt()`, which is the better design.
  Update the doc.

- [x] DESIGN.md §7 says enforcement reacts to `tabs.onActivated`; only `onCreated` and
