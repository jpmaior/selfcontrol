...
- **`nextChangeAtMs` is the earliest instant the answer could flip:** `now + remainingMs`
  ...
  its own. Later sections add a scheduled block's start and a pass's end to the candidates.
...
instant, so the toolbar badge (§18) can flip without a user event, and (from §15) a
scheduled rule holds one even while nothing is counting.
